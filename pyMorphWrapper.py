...
class MorphingWrapper:
    ...
    def pre2DDoubleBasis(self, c_min, c_max, npoints, indexes=None):
        '''
        ...
        # Change this to indexes pre computed
        if indexes == None:
            indexes = self.dynamicMorphing_(
                self.F1, csarray_0, csarray_1,alpha=False)
        target = self.F1[:]
            # Start computing couplings and cross sections
        for l, ind in enumerate(indexes):
            ind = np.array(ind)
            self.resetBasis([self.samples[int(k)] for k in ind])
            sorted_indexes = np.argsort(ind)
            indexes[l] = ind[sorted_indexes]
            for i, cs in enumerate(csarray_0):
                for j, cs2 in enumerate(csarray_1):
                    target[1] = cs
                    target[2] = cs2
                    self.resetTarget(target)
                    couplings = np.array(self.getWeights())
                    cross_section = np.array(self.getCrossSections())
                    couplings, cross_section = (couplings[sorted_indexes],
                                                cross_section[sorted_indexes])
                    all_couplings = np.vstack(
                        [all_couplings, couplings]) if i != 0 or j != 0 or l != 0 else couplings
                    all_cross_sections = np.vstack(
                        [all_cross_sections, cross_section]) if i != 0 or j != 0 or l != 0 else cross_section
        ...
        np.savetxt('doubleindexes_{0:.2f}_{1:.2f}_{2:.2f}_{3:.2f}_{4}.dat'.format(
            c_min[0], c_min[1], c_max[0], c_max[1], npoints), indexes)
        np.savetxt('doublecouplings_{0:.2f}_{1:.2f}_{2:.2f}_{3:.2f}_{4}.dat'.format(
            c_min[0], c_min[1], c_max[0], c_max[1], npoints), all_couplings)
        np.savetxt('doublecrosssection_{0:.2f}_{1:.2f}_{2:.2f}_{3:.2f}_{4}.dat'.format(
            c_min[0], c_min[1], c_max[0], c_max[1], npoints), all_cross_sections)
